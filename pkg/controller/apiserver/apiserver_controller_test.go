...
var _ = Describe("apiserver controller tests", func() {
	...
	BeforeEach(func() {
		...
		Expect(err).NotTo(HaveOccurred())
		packetCaptureSecret, err = secret.CreateTLSSecret(cryptoCA, render.PacketCaptureServerCert(false, "").Name(), common.OperatorNamespace(), "key.key", "cert.crt", time.Hour, nil, dns.GetServiceDNSNames(render.PacketCaptureServiceName, render.PacketCaptureNamespace, dns.DefaultClusterDomain)...)
		Expect(err).NotTo(HaveOccurred())
	...
	Context("verify reconciliation", func() {
		...
		It("should not add OwnerReference to user-supplied apiserver and packetcapture TLS cert secrets", func() {
			...
			Expect(cli.Get(ctx, client.ObjectKey{Namespace: common.OperatorNamespace(), Name: render.PacketCaptureServerCert(false, "").Name()},
				packetCaptureSecret2)).ShouldNot(HaveOccurred())
